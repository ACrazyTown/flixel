...
class FlxTrianglesData implements IFlxDestroyable
{
	 ...
	 */
	static var tempBounds:FlxRect = new FlxRect();

	static var tempPoint:FlxPoint = new FlxPoint();

	static var sprite:Sprite = new Sprite();
	...
	public static function fromTexturePackerXml(Description:String, TextureSize:FlxPoint):StringMap<FlxTrianglesData>
	{
		...
		var fast:Access = new Access(Xml.parse(Description).firstElement());
	 ...
	 * To increase `indexCapacity` you should call `extendIndices()` method.
	 */
	...
	var uvsArray:Float32Array;
	var colorsArray:UInt32Array;
	var indicesArray:UInt16Array;

	var verticesBuffer:GLBuffer;
	var uvsBuffer:GLBuffer;
	var colorsBuffer:GLBuffer;
	...
	public function updateBounds():Void
	{
		if (verticesDirty && vertices.length >= 6)
		{
			...
			while (i < numVertices)
			{
				bounds.unionWithPoint(FlxPoint.get(vertices[i], vertices[i + 1]));
				i += 2;
			}
		}
	}
	 ...
	 */
	public function getBitmapData(pixels:BitmapData, graphics:FlxGraphic, color:FlxColor):BitmapData
	{
		updateBounds();
		...
		else
		{
			...
			// pixels.clearColor(FlxColor.TRANSPARENT);
		}

		sprite.graphics.clear();

		if (graphics != null)
			sprite.graphics.beginBitmapFill(graphics.bitmap);
		else
	...
	function set_vertices(value:Vector<Float>):Vector<Float>
	{
	...
	function set_uvs(value:Vector<Float>):Vector<Float>
	{
	...
	function set_colors(value:Vector<FlxColor>):Vector<FlxColor>
	{
	...
	function set_indices(value:Vector<Int>):Vector<Int>
	{
	...
	function set_verticesDirty(value:Bool):Bool
	{
		return verticesDirty = value;
	}

	function set_uvtDirty(value:Bool):Bool
	{
		return uvtDirty = value;
	}

	function set_colorsDirty(value:Bool):Bool
	{
		return colorsDirty = value;
	}

	function set_indicesDirty(value:Bool):Bool
	{
	...
	function updateVertices():Void
	{
		...
		if (verticesDirty)
		{
			...
			GLHelper.bufferData(GL.ARRAY_BUFFER, verticesArray, GL.STATIC_DRAW);
			verticesDirty = false;
		}
		else
		{
	...
	function updateUV():Void
	{
		...
		if (uvtDirty)
		{
			...
			GLHelper.bufferData(GL.ARRAY_BUFFER, uvsArray, GL.STATIC_DRAW);
			uvtDirty = false;
		}
		else
		{
	...
	function updateColors():Void
	{
		...
		if (colorsDirty)
		{
			...
			GLHelper.bufferData(GL.ARRAY_BUFFER, colorsArray, GL.STATIC_DRAW);
			colorsDirty = false;
		}
		else
		{
	...
	function updateIndices():Void
	{
		...
		if (indicesDirty)
		{
			...
			GLHelper.bufferData(GL.ELEMENT_ARRAY_BUFFER, indicesArray, GL.STATIC_DRAW);
			indicesDirty = false;
		}
		else
		{
